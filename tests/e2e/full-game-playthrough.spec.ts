...
test('Full campaign reaches hidden reef and leaderboard', async ({ page }) => {
  await page.goto('/');
  ...
  const playIsland = async (islandId: string): Promise<void> => {
    await page.evaluate((id) => {
      (window as { __dr_debug?: { sailToIsland: (islandId: string) => void } }).__dr_debug?.sailToIsland(id);
    }, islandId);
    ...
    await page.waitForFunction(() => {
      const scene = (window as { __dr_scene?: string }).__dr_scene;
      return scene === 'overworld' || scene === 'leaderboard';
    });
  };
  ...
  for (const islandId of ['island_01', 'island_02', 'island_03', 'island_04', 'island_05']) {
    await playIsland(islandId);
    const scene = await page.evaluate(() => (window as { __dr_scene?: string }).__dr_scene);
    expect(scene).toBe('overworld');
  }

  await playIsland('hidden_reef');
